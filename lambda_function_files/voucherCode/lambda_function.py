...
def lambda_handler(event, context):
    ...
    # Get serialNumber from event
    serial_number = event.get('serialNumber')
    if not serial_number:
        return {
            'statusCode': 400,
            'body': json.dumps('serialNumber is required.')
        }

    # URL to make the GET request
    url = f'https://flairair-api.intelisystraining.ca/RESTv1/vouchers?serialNumber={serial_number}'
